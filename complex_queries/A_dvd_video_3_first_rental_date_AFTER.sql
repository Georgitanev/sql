...
select min(rental_id) as first_rental_id
from rental
where customer_id in ( 70, 588) -- 70 - 2005-05-31 06:24:44
group by
	customer_id -- 70	2005-05-31 06:24:44

------------------------------------------------
select rental.customer_id as customer1,
...
-----------------------
with fisrt_watched_movie as (	select rental.customer_id as customer1,
										film.title as first_movie
								from rental
								join inventory on (inventory.inventory_id = rental.inventory_id) 
								join film on (film.film_id = inventory.film_id)
...
last_watched_movie as (	select rental.customer_id as customer2,
...
select fisrt_watched_movie.customer1 as customer_num, fisrt_watched_movie.first_movie, last_watched_movie.last_movie  
from fisrt_watched_movie
join last_watched_movie on (last_watched_movie.customer2 = fisrt_watched_movie.customer1)
